...
import { HtmlJson, renderToStream } from './html';
...
export function createProgram(filePath: string): { program: ts.Program; sourceFile: ts.SourceFile; checker: ts.TypeChecker } | null {
  try {
...
export function getTypeName(typeAlias: ts.TypeAliasDeclaration): string | null {
  if (ts.isTypeReferenceNode(typeAlias.type) && typeAlias.type.typeName &&
